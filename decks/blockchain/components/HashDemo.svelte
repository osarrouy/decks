...
>
  <article class="sample original framed crossed">
    <p class="label">Texte A</p>
    ...
    <div class="digest" class:visible={phase >= 1}>
      <span>SHA-256</span>
      <code>
        <span>{originalHash.slice(0, 32)}</span>
        <span>{originalHash.slice(32)}</span>
      </code>
    </div>
  </article>

  <article class="sample changed framed crossed" class:visible={phase >= 2}>
    <p class="label">Texte B · 1 caractère modifié</p>
    <blockquote>
    ...
    <div class="digest" class:visible={phase >= 3}>
      <span>SHA-256</span>
      <code>
        <span>{changedHash.slice(0, 32)}</span>
        <span>{changedHash.slice(32)}</span>
      </code>
    </div>
  </article>
</figure>
...
<style>
  ...
  .sample {
    position: relative;
    display: grid;
    grid-template-rows: auto minmax(0, 1fr) 3.8em 4.6em;
    min-width: 0;
    padding: var(--space-4);
