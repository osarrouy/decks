...
- Le deck doit être créé dans `slides/economics/`.
- La démo existante dans `slides/demo/` est une référence de système et de syntaxe, **pas un fichier à modifier**.
- Le nouveau projet doit utiliser le moteur `svx-slides`, son format `deck.svx`, son `deck.config.ts`, ses assets et ses notes speaker.
- Le lancement attendu est `pnpm dev:economics`.
- Le script racine doit pointer vers `slides/economics` sans modifier les scripts de la démo, sauf ajout indépendant strictement nécessaire.
...
- Les fichiers générés (`.svx-slides/`, `build/`) ne doivent pas servir de source éditoriale.
