...
test('subpath exports prefix real assets across Markdown, component props and CSS', async () => {
  ...
  try {
    ...
    assert.equal(prefixStaticAssets('![Alt](/image.png) <Persona picture="/image.png" /> url(/image.png?v=1#detail)', root, '/slides/history'),
      '![Alt](/slides/history/image.png) <Persona picture="/slides/history/image.png" /> url(/slides/history/image.png?v=1#detail)')
    const external = '<img src="https://example.org/image.png" /><a href="/chapter">route</a> url(//cdn.example.org/image.png)'
    assert.equal(prefixStaticAssets(external, root, '/slides/history'), external)
