...
const directiveLayoutNames = new Set([
  "columns",
  "two-columns",
  "center",
  "cover",
...
function mergeProperties(node, properties) {
  ...
  if (incomingClasses) {
    ...
    const previousClasses = Array.isArray(previous)
      ? previous
      : previous
        ? String(previous).split(/\s+/)
        : [];
  ...
  return (tree) => {
    visit(tree, (node) => {
      ...
      for (let index = 0; index < node.children.length; index += 1) {
        ...
        if (!child || child.type !== "image" || !next || next.type !== "text")
          continue;
...
function directiveClasses(name, attributes = {}) {
  ...
  if (attributes.class)
    classes.push(...String(attributes.class).split(/\s+/).filter(Boolean));
  if (attributes.className)
    classes.push(...String(attributes.className).split(/\s+/).filter(Boolean));
...
function htmlAttributesForProperties(properties) {
  ...
  const className = Array.isArray(copy.className)
    ? copy.className.join(" ")
    : "";
  ...
  const attributes = Object.entries(copy)
    .map(([key, value]) =>
      value === true ? key : `${key}=${JSON.stringify(String(value))}`,
    )
    .join(" ");
...
export function svxDeckMarkdownPreprocess() {
  return {
    name: "svx-deck-markdown-preprocess",
...
export function svxDeckMdsvexOptions(options = {}) {
  return {
    ...options,
    remarkPlugins: [
