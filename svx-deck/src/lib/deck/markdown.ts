...
const noteDirectiveHtml = directiveHtml({
  ...
  comment(directive) {
    this.raw(
      `<blockquote class="presenter-note-comment">${directive.content || ""}</blockquote>`,
    );
    return true;
  },
  example(directive) {
    this.raw(
      `<blockquote class="presenter-note-example">${directive.content || ""}</blockquote>`,
    );
    return true;
  },
  warning(directive) {
    this.raw(
      `<blockquote class="presenter-note-warning">${directive.content || ""}</blockquote>`,
    );
    return true;
  },
});
...
function hidePresenterNotes(html: string) {
  return html.replace(
    /<blockquote class="presenter-note-comment">[\s\S]*?<\/blockquote>/g,
    "",
  );
}
...
export function notesToHtml(
  ...
  return options.audience === "student"
    ? hidePresenterNotes(decorated)
    : decorated;
}
