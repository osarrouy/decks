...
- The CLI dispatches commands, `src/runtime/` generates apps, and `src/lib/` owns compilation and rendering. Edit these sources, never generated `.svx-deck.nosync/` apps.
- Maintain one parser implementation used by both the CLI and Vite plugin.
- Keep slide surface styles in `theme/default.css`, imported by `SlideSurface.svelte` and scoped under `.slide[data-slide-surface]`. Other components own their layout and styles. Shared values, crosses and controls come from `@dg/ui`; do not add theme packages, a registry or per-deck visual selection.
